...
(function () {
  ...
  Panels.App = Backbone.Router.extend({
    ...
    commitLayout: function (instant) {
      ...
      if (goingBack) {
        ...
        for (var i = Math.max(this.j, this.targetI + 1), end = this.i; i <= end; i++) {
          ...
          this.panels[i].moveTo(
            [totalWidth, this.getPanelWidth(this.panels[i]), "auto"],
            {
              hide: true,
            },
            instant,
      ...
      if (this.targetLeftGap) {
        buffer +=
          '<a class="pfx-go-left" style="width:' +
          (this.targetLeftGap + this.goLeftWidthOffset) +
          'px" href="' +
          this.root +
          this.panels[this.j - 1].fragment +
          '"></a>';
      }
      if (this.targetRightGap) {
        buffer +=
          '<a class="pfx-go-right" style="width:' +
          (this.targetRightGap + this.goRightWidthOffset) +
          'px" href="' +
          this.root +
          this.panels[this.i + 1].fragment +
          '"></a>';
      }
      if (buffer) {
        if (instant) {
  ...
  Panels.StaticPanel = Panel.extend({
    ...
    load: function () {
      ...
      $.get(this.app.root + this.fragment + (this.fragment.indexOf("?") >= 0 ? "&" : "?") + "output=html").done(
        function (content) {
          this.html(content);
          this.loaded = true;
          this.whenLoadedQueueFlush();
    ...
    toString: function (t) {
      ...
      for (var r in this) {
        if (this.hasOwnProperty(r)) {
          if (
            !q.transform3d &&
            (r === "rotateX" || r === "rotateY" || r === "perspective" || r === "transformOrigin")
          ) {
            continue;
          }
          if (r[0] !== "_") {
